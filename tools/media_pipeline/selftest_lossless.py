#!/usr/bin/env python3
"""Fast end-to-end self-test for OdiumFlix lossless HLS packaging."""
...
def assert_hls_source(source: Path, output: Path, *, processing_mode: str) -> dict:
    manifest = process_item(
        BatchItem(
            ...
            audio_codec="copy", processing_mode=processing_mode,
        ),
    ...
    assert qualities[0]["playlist"].endswith("video.m3u8")
    assert manifest["playback"]["mode"] == "hls", manifest["playback"]
    assert manifest["playback"]["master"] == "playback.m3u8"
...
def main() -> None:
    ...
    try:
        ...
        manifest = assert_hls_source(source, root / "out-with-tracks", processing_mode="split")
        ...
        run([
            command("ffmpeg"), "-hide_banner", "-loglevel", "error", "-y",
            "-f", "lavfi", "-i", "testsrc2=size=320x180:rate=24:duration=1.5",
            "-c:v", "libx264", "-preset", "ultrafast", "-crf", "18", "-an", str(silent),
        ])
        silent_manifest = assert_hls_source(silent, root / "out-silent-auto", processing_mode="auto")
        assert silent_manifest["playback"]["mode"] == "hls"
        assert silent_manifest["playback"]["directFile"] is None
        print("LOSSLESS_HLS_SELFTEST_OK")
    finally:
