...
app = FastAPI(title="OdiumFlix Media Worker", version="0.3.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
async def create_batch(
    ...
    (incoming / "plan.json").write_text(
        json.dumps({"schemaVersion": 2, "items": items, "settings": settings}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    JOBS[job_id] = {
        "id": job_id,
...
def merge_prepared_items(values: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for original in values:
        ...
        current["target_qualities"] = list(dict.fromkeys([
            *(current.get("target_qualities") or []),
            *(value.get("target_qualities") or []),
        ]))
...
def run_job(job_id: str, raw_values: list[dict[str, Any]], settings: dict[str, Any]) -> None:
    ...
    try:
        JOBS[job_id].update(status="processing", progress=5, message="Yerel medya analizi başladı")
        ...
        for index, value in enumerate(values):
            ...
            JOBS[job_id].update(
                ...
                message=f"{index + 1}/{len(values)} medya yerelde işlendi",
            )
        ...
        if hf_repo:
            ...
            hf_commits = upload_huggingface(
                ...
                token=hf_token,
                private=hf_private,
                message=f"OdiumFlix mobile batch {job_id}",
            )
